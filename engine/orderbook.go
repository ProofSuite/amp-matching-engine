...
func (ob *OrderBook) buyOrder(o *types.Order) (*types.EngineResponse, error) {
	...
	for _, mo := range matchingOrders {
		...
		match := &types.OrderTradePair{mo, trade}
		matches.AppendMatch(match)
...
func (ob *OrderBook) DeleteOrder(o *types.Order) (*types.EngineResponse, error) {
	ob.mutex.Lock()
	defer ob.mutex.Unlock()

	//TODO add the delete order in the order dao
	// err := ob.orderDao.DeleteOrder(o.Hash)
	// if err != nil {
	// 	logger.Error(err)
	// 	return nil, err
	// }
